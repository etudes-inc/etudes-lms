/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/courier/courier-tool/tool/src/java/org/sakaiproject/courier/tool/CourierTool.java $
 * $Id: CourierTool.java 8044 2014-05-29 22:13:23Z ggolden $
 ***********************************************************************************
 *
...
//			}
			...
			// if we have no user (no login), "gateway" the request
			if (session.getUserId() == null)
			{
				gatewayRequest(session, placementId, req, res);
			}
			else
			{
				// if this is a site presence request, check permission
				if (placementId.endsWith("-presence"))
				{
					String siteId = placementId.substring(0, placementId.length() - "-presence".length());
					boolean ok = SecurityService.unlock(session.getUserId(), "site.visit", SiteService.siteReference(siteId));
					if (!ok)
					{
						gatewayRequest(session, placementId, req, res);
						return;
				...
				// find all deliveries for the requested delivery address
	...
	/**
	 * Cause the browser to return to the gateway.
	 * @param session
	 * @param placementId
	 * @param req
	 * @param res
	 ...
	 * @throws IOException
	 */
	protected void gatewayRequest(Session session, String placementId, HttpServletRequest req, HttpServletResponse res)  throws ServletException, IOException
	{
		String loggedOutUrl = ServerConfigurationService.getLoggedOutUrl();
		if (M_log.isDebugEnabled()) M_log.debug("sending top redirect: " + placementId + " : " + loggedOutUrl);
