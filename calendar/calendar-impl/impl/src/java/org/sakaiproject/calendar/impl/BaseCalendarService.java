/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/calendar/calendar-impl/impl/src/java/org/sakaiproject/calendar/impl/BaseCalendarService.java $
 * $Id: BaseCalendarService.java 5933 2013-09-12 18:32:58Z ggolden $
 ***********************************************************************************
 *
...
public abstract class BaseCalendarService implements CalendarService, StorageUser, CacheRefresher, ContextObserver, EntityTransferrer
{
	...
	*/
    ...
    public Date getMinStartDate(String course_id)
	{
			...
			String calRef = calendarReference(course_id, SiteService.MAIN_CONTAINER);
			Calendar cal = null;
			Date minStartDate = null;
			Time minStartTime = null;
			try
			{
				cal = (Calendar) getCalendar(calRef);
				...
				while (events.hasNext())
				{
					...
					if (event.getRange().firstTime() != null)
					{	
					  timeList.add(event.getRange().firstTime());
					}  
			...
			{
				M_log.warn(".getMinStartDate: error for service: " + CalendarService.class.getName() + " channel: "
						+ calRef);
				return null;
			}
    ...
    public Date getMaxStartDate(String course_id)
	{
			...
			{
				M_log.warn(".getMaxStartDate: error for service: " + CalendarService.class.getName() + " channel: "
						+ calRef);
				return null;
			}
	 ...
	 */    
	 public void applyBaseDateTx(String course_id, int days_diff)
	 {
		    if (course_id == null)
			{
				M_log.warn("calendar: applyBaseDateTx: course_id is null");
			}
			if (days_diff == 0)
			{
				return;
			...
			// get the channel associated with this site
		    String calRef = calendarReference(course_id, SiteService.MAIN_CONTAINER);
			Calendar oCal, nCal = null;
			GregorianCalendar gc = new GregorianCalendar();
			List idList = new ArrayList();
			try
			{
				oCal = (Calendar) getCalendar(calRef);

				if (oCal != null)
				{
					Iterator events = oCal.getEvents(null, null).iterator();
					while (events.hasNext())
					{
						idList.add(((CalendarEvent)events.next()).getId());
					}
				}	// if
			}
			catch (Exception any)
			{
				M_log.warn(".applyBaseDateTx: error for service: " + CalendarService.class.getName() + " channel: "
						+ calRef);
			}
			try
			{
				nCal = (Calendar) getCalendar(calRef);
				...
				if (nCal != null)
				{
					if (idList != null)
				    {
					  if (idList.size() > 0)
				      {
				    	...
				    	while (i.hasNext())
				    	{
				    	  CalendarEventEdit eventEdit = (CalendarEventEdit) nCal.getEditEvent((String)i.next(),org.sakaiproject.calendar.api.CalendarService.EVENT_MODIFY_CALENDAR);
				    	  Time releaseTime = null,retractTime = null;
				    	  TimeRange newTimeRange = null;
						  try
						  {
						    if (eventEdit.getRange().firstTime() != null)
						    {
							Date releaseDate = new Date();
							releaseDate.setTime(eventEdit.getRange().firstTime().getTime());
							gc.setTime(releaseDate);
			    			gc.add(java.util.Calendar.DATE, days_diff);
			    			releaseTime = TimeService.newTime(gc);
			    		    }
					      }
					      catch (Exception e)
					      {
						  //If release date isn't defined, skip setting
					      }
					      try
						  {
						    if (eventEdit.getRange().lastTime() != null)
						    {
							Date retractDate = new Date();
							retractDate.setTime(eventEdit.getRange().lastTime().getTime());
							gc.setTime(retractDate);
			    			gc.add(java.util.Calendar.DATE, days_diff);
			    			retractTime = TimeService.newTime(gc);
			    		    }
					      }
					      catch (Exception e)
					      {
						  //If retract date isn't defined, skip setting
					      }
					      if ((releaseTime != null)||(retractTime != null))
					      {
					    	  newTimeRange = TimeService.newTimeRange(releaseTime, retractTime);
					      }
					      if (newTimeRange != null)
					      {
					    	  eventEdit.setRange(newTimeRange);
					      }
					      nCal.commitEvent(eventEdit);
					     }
					}
				}
				}	// if
	...
	public void transferCopyEntities(String fromContext, String toContext, List resourceIds)
	{
		...
		try
		{
			...
			if (nCalendar != null)
			{
				// get a set of the destination (n - new) calendar's fields
				Set<String> nFieldsSet = new HashSet<String>();
				String nFields = StringUtil.trimToZero(nCalendar.getEventFields());
				String[] nFieldsArray = StringUtil.split(nFields, ADDFIELDS_DELIMITER);
				for (String f : nFieldsArray)
				{
					nFieldsSet.add(f);
				}

				// get the fields from the source (o - old) calendar - just as an array
				String oFields = StringUtil.trimToZero(oCalendar.getEventFields());
				String[] oFieldsArray = StringUtil.split(oFields, ADDFIELDS_DELIMITER);

				// build up the combined set - existing (n) ones first, then any ones from the source (o) that don't already exist in the destination
				for (String f : oFieldsArray)
				{
					if (nFieldsSet.contains(f)) continue;

					if (nFields.length() > 0) nFields += ADDFIELDS_DELIMITER;
					nFields += f;
					
					// add to the destination fields set to avoid bringing in dups that may already be in the source
					nFieldsSet.add(f);
				}
				
				// set the fields
				nCalendar.setEventFields(nFields);
				...
				for (int i = 0; i < oEvents.size(); i++)
				{
					...
					{
						// process embed data
						String nDescriptionData = oEvent.getDescriptionFormatted();
						Set<String> refs = XrefHelper.harvestEmbeddedReferences(nDescriptionData, null);
						M_log.debug("embed data found:" + refs.toString());
						if (!refs.isEmpty())
						{
							List<Translation> translations = XrefHelper.importTranslateResources(refs, toContext, "Schedule");
							nDescriptionData = XrefHelper.translateEmbeddedReferences(nDescriptionData, translations, toContext);
						}

						CalendarEvent e = nCalendar.addEvent(oEvent.getRange(), oEvent.getDisplayName(), oEvent.getDescription(), oEvent.getType(),
								oEvent.getLocation(), oEvent.getAttachments());
						...
						try
						{
							...
							eEdit.setDescriptionFormatted(nDescriptionData);

							// recurrence rule
							RecurrenceRule rule = oEvent.getRecurrenceRule();
							eEdit.setRecurrenceRule(rule);
							eEdit.setExclusionRule(((BaseCalendarEventEdit) oEvent).getExclusionRule());
							...
							try
							{
								...
								for (Group oGroup : (Collection<Group>) oEvent.getGroupObjects())
								{
									for (Group g : groups)
									{
								...
								{
									eEdit.setGroupAccess(eventGroups, true);
								}
							}
							...
							if (nCalendar.containsMatchingEvent(eEdit))
							{
								nCalendar.removeEvent(eEdit);
							}
											...
											try
											{
												...
												try
												{
													if (ContentHostingService.isAttachmentResource(nAttachmentId))
													{
														...
														ContentResource attachment = ContentHostingService.addAttachmentResource(oAttachment
																.getProperties().getProperty(ResourceProperties.PROP_DISPLAY_NAME), toContext,
																ToolManager.getTool("sakai.schedule").getTitle(), oAttachment.getContentType(),
																oAttachment.getContent(), oAttachment.getProperties());

														// harvest any embedded references into resources
														XrefHelper.harvestTranslateResource(attachment, toContext, "Schedule");
													...
													else
													{
														...
														ContentResource attachment = ContentHostingService.addResource(oAttachment.getProperties()
																.getProperty(ResourceProperties.PROP_DISPLAY_NAME), toContext, 1, oAttachment
																.getContentType(), oAttachment.getContent(), oAttachment.getProperties(),
	...
	public class BaseCalendarEdit extends Observable implements CalendarEdit, SessionBindingListener
	{
		...
		public boolean containsMatchingEvent(CalendarEvent event)
		{
			try
			{
				List<CalendarEvent> events = getEvents(null, null);
				for (CalendarEvent e : events)
				{
					if (e.getId().equals(event.getId())) continue;

					if (different(e.getDisplayName(), event.getDisplayName())) continue;
					// Note: trim, because if an existing entry has trailing white space, older archives (<2014) will have had that trimmed -ggolden					
					if (different(e.getDescriptionFormatted().trim(), event.getDescriptionFormatted().trim())) continue;
					if (different(e.getType(), event.getType())) continue;
					if (!e.getRange().equals(event.getRange())) continue;
					if (different(e.getLocation(), event.getLocation())) continue;
					if (different(e.getRecurrenceRule(), event.getRecurrenceRule())) continue;

					// groups
					Collection<String> eGroups = e.getGroups();
					Collection<String> eventGroups = event.getGroups();
		...
		public void commitEvent(CalendarEventEdit edit, int intention)
		{
         ...
         edit.setModifiedBy();  // update modified-by properties
			...
			// clear thread local cache of events that may include this
			ThreadLocalManager.set(getReference() + ".events", null);
	...
	public class BaseCalendarEventEdit implements CalendarEventEdit, SessionBindingListener
	{
		...
		public String getDescriptionFormatted()
		{
			...
			if (desc != null && desc.length() > 0) return HtmlHelper.clean(desc);
