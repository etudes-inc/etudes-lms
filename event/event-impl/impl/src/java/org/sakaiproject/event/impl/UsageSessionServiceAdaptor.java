/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/event/event-impl/impl/src/java/org/sakaiproject/event/impl/UsageSessionServiceAdaptor.java $
 * $Id: UsageSessionServiceAdaptor.java 2470 2012-01-09 19:52:04Z ggolden $
 ***********************************************************************************
 *
...
public abstract class UsageSessionServiceAdaptor implements UsageSessionService
{
	...
	public boolean login(Authentication authn, HttpServletRequest req)
	{
		...
		// form a "resource" string for the event that records the sakai session id
		String resource = "session:" + sakaiSession.getId();

		// post the login event
		eventTrackingService().post(eventTrackingService().newEvent(EVENT_LOGIN, resource, true));
	 ...
	 */
	protected void logoutEvent(UsageSession session, boolean byUser)
	{
		// TODO: put:
		// /** Name for the event of automatically logging out. */
		// public static final String EVENT_AUTO_LOGOUT = "auto.logout";
		// in UsageSessionService api, and use it instead of the constant -GGOLDEN
		String eventCode = byUser ? EVENT_LOGOUT : "auto.logout";

		if (session == null)
		{
			// generate a logout event (current session)
			eventTrackingService().post(eventTrackingService().newEvent(eventCode, null, true));
		}
		else
		{
			// generate a logout event (this session)
			eventTrackingService().post(eventTrackingService().newEvent(eventCode, null, true), session);
		}
	}
	...
	protected class BaseUsageSession implements UsageSession, SessionBindingListener
	{
		...
		public void valueUnbound(SessionBindingEvent sbe)
		{
			...
			if (!isClosed())
			{
				...
				// TODO: add this.endedByUser, set by setEndedByUser in UsageSession api
				boolean endedByUser = "true".equals(threadLocalManager().get("LoginTool:userLogout"));

				// generate the logout event
				logoutEvent(this, endedByUser);
			}
		}
