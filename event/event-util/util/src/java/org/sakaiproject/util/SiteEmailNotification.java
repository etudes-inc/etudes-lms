/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/event/event-util/util/src/java/org/sakaiproject/util/SiteEmailNotification.java $
 * $Id: SiteEmailNotification.java 1966 2011-09-01 20:21:37Z mallikamt $
 ***********************************************************************************
 *
...
public class SiteEmailNotification extends EmailNotification
{
	...
	protected List getRecipients(Event event)
	{
		...
		//If site is unpublished and the priority is high, use the list of users who can SITE_VISIT the site
		// else use the list of users who can SITE_VISIT_UNP the site.
		try
		{
			Site site = SiteService.getSite(siteId);
			String ability = SiteService.SITE_VISIT;
			if (!site.isPublished() && (event.getPriority() != NotificationService.NOTI_REQUIRED))
			{
	...
	protected String getToSite(Event event)
	{
		...
		// ETUDES: removed following for SAK-22
//		try
//		{
//			Site site = SiteService.getSite(siteId);
//			title = site.getTitle();
