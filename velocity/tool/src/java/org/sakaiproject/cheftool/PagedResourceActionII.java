/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/velocity/tool/src/java/org/sakaiproject/cheftool/PagedResourceActionII.java $
 * $Id: PagedResourceActionII.java 11617 2015-09-15 19:27:59Z mallikamt $
 ***********************************************************************************
 *
...
public abstract class PagedResourceActionII extends VelocityPortletPaneledAction
{
	...
	protected List prepPage(SessionState state)
	{
		...
		else if (goLastPage)
		{
			...
			{
				posStart = (totalPageNumber - 1)* pageSize;
			} else posStart = 0;
			if (posStart < 0) posStart = 0;
		}
	...
	public void doChange_pagesize(RunData data, Context context)
	{
		...
		// from second page, if pagesize changes then reset
		resetPaging(state);
	} // doChange_pagesize
