/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/util/util-util/util/src/java/org/sakaiproject/util/FormattedText.java $
 * $Id: FormattedText.java 9419 2014-12-02 23:17:24Z ggolden $
 ***********************************************************************************
 *
...
public class FormattedText
{
	...
	public static String escapeHtml(String value, boolean escapeNewlines)
	{
		...
		try
		{
			...
			for (int cp, i = 0; i < value.length(); i += Character.charCount(cp))
			{
					...
					&& (value.charAt(i + 1) == '#' || value.charAt(i + 1) == '^'))
			{
				...
				if (pos < valuelength)
				{
					try
					{
						...
						buf.append(Character.toChars(val));
						i = pos;
					}
