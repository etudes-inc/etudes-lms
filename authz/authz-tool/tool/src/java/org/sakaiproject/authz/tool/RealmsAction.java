/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/authz/authz-tool/tool/src/java/org/sakaiproject/authz/tool/RealmsAction.java $
 * $Id: RealmsAction.java 720 2010-09-13 19:08:47Z ggolden $
 ***********************************************************************************
 *
	...
	private boolean readUserForm(RunData data, SessionState state)
	{
		...
		if (user == null)
		{
			...
			try
			{
				...
				user = UserDirectoryService.getUser(eid);
			}
			catch (UserNotDefinedException e)
			{
				// try by eid and institution code
				try
				{
