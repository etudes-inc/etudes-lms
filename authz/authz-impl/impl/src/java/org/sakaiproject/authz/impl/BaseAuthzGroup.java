/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/authz/authz-impl/impl/src/java/org/sakaiproject/authz/impl/BaseAuthzGroup.java $
 * $Id: BaseAuthzGroup.java 7652 2014-03-19 22:32:13Z ggolden $
 ***********************************************************************************
 *
...
public class BaseAuthzGroup implements AuthzGroup
{
	...
	public Set getActiveMembers()
	{
		...
		for (Iterator it = m_userGrants.entrySet().iterator(); it.hasNext();)
		{
			Map.Entry entry = (Map.Entry) it.next();
			BaseMember grant = (BaseMember) entry.getValue();
			if (grant.active && grant.role.isAllowed("site.visit"))
			{
				rv.add(grant);
			}
		}
	...
	public void addMember(String user, String roleId, boolean active, boolean provided)
	{
		...
		if (role == null)
		{
			M_log.warn("addMember: role undefined: " + roleId + " user: " + user + " authzId: " + m_id);
			return;
		}
	...
	public boolean keepIntersection(AuthzGroup other)
	{
		...
		for (Iterator it = m_userGrants.entrySet().iterator(); it.hasNext();)
		{
			...
			else
			{
				...
				// make sure we have the same role as the other
				if (!grant.getRole().getId().equals(otherMember.getRole().getId()))
				{
					// change role, if the group has a role defined to match the site
					Role role = (Role) m_roles.get(otherMember.getRole().getId());
					if (role != null)
					{
						((BaseMember) grant).role = otherMember.getRole();
						rv = true;
					}
				}
			}
