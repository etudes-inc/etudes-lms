/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/web/web-tool/tool/src/java/org/sakaiproject/web/tool/IFrameAction.java $
 * $Id: IFrameAction.java 8546 2014-08-29 23:12:15Z rashmim $
 ***********************************************************************************
 *
...
public class IFrameAction extends VelocityPortletPaneledAction
{
	...
	protected void initState(SessionState state, VelocityPortlet portlet, JetspeedRunData rundata)
	{
		...
		state.setAttribute("popNewWindow", new Boolean(p.isPopUp()).toString());
	}
	...
	protected String sourceUrl(String special, String source, String context, boolean macroExpansion, boolean passPid, String pid, String sakaiPropertiesUrlKey)
	{
		...
		Placement placement = SiteService.findTool(pid);
		...
		// LTI link
		else if (placement.getConfig().getProperty("key") != null && placement.getConfig().getProperty("secret") != null)
		{
			SitePage p = SiteService.findPage(getCurrentSitePageId());
			String resource_link_id = p.getUrl();
			resource_link_id = resource_link_id.substring(resource_link_id.lastIndexOf("/")+1);
	
			rv = "/access/lticontact/"+ placement.getTitle()+"/"+context+"/"+new Boolean(p.isPopUp()).toString()+"/"+resource_link_id+"/";
		}
