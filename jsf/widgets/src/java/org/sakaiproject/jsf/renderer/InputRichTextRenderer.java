/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/jsf/widgets/src/java/org/sakaiproject/jsf/renderer/InputRichTextRenderer.java $
 * $Id: InputRichTextRenderer.java 8546 2014-08-29 23:12:15Z rashmim $
 ***********************************************************************************
 *
 ...
 * </p>
 * <p>
 * HTMLArea is a free, customizable online editor. It works inside your browser. It uses a non-standard feature implemented in Internet Explorer 5.5 or better for Windows and Mozilla 1.3 or better (any platform), therefore it will only work in one of
 * these browsers.
 * </p>
 * 
 * <p>
 * HTMLArea is copyright <a href="http://interactivetools.com">InteractiveTools.com</a> and released under a BSD-style license. HTMLArea is created and developed upto version 2.03 by InteractiveTools.com. Version 3.0 developed by <a
 * href="http://students.infoiasi.ro/~mishoo/">Mihai Bazon</a> for InteractiveTools. It contains code sponsored by other companies as well.
 * </p>
 * 
 * <p>
 ...
 * @version $Id: InputRichTextRenderer.java 8546 2014-08-29 23:12:15Z rashmim $
 */
public class InputRichTextRenderer extends Renderer
{
	...
	public void encodeBegin(FacesContext context, UIComponent component) throws IOException
	{
		...
		String value = null;
		if (component instanceof UIInput) value = (String) ((UIInput) component).getSubmittedValue();
		if (value == null && component instanceof ValueHolder) value = (String) ((ValueHolder) component).getValue();
		...
		RendererUtil.writeExternalJSDependencies(context, writer, "inputrichtext.jsf.ckeditor.js", ServerConfigurationService.getString("etudes.editor.path")+"/ckeditor/ckeditor.js");
		RendererUtil.writeExternalJSDependencies(context, writer, "inputrichtext.jsf.ckeditorlaunch.js", ServerConfigurationService.getString("etudes.editor.path")+"/ckeditor.launch.js");
		...
		if (widthPx == DEFAULT_WIDTH_PX && textareaColumns != DEFAULT_COLUMNS) widthPx = (DEFAULT_WIDTH_PX * textareaColumns) / DEFAULT_COLUMNS;
		if (heightPx == DEFAULT_HEIGHT_PX && textareaRows != DEFAULT_ROWS) heightPx = (DEFAULT_HEIGHT_PX * textareaRows) / DEFAULT_ROWS;
		...
		{
			collectionId = collectionBase.replaceAll("\"", "\\\"");
			SessionManager.getCurrentSession().setAttribute("ck.collectionId", collectionId);
		}

		String resourceid = (String) RendererUtil.getAttribute(context, component, "resourceId");
		...
		if (value != null) writer.write(Validator.escapeHtmlTextarea((String) value));
		...
		if (!"true".equals(textareaOnly))
		{
			...
			if (showVTInEditor)
			{				
				writer.write("config.prototype.serverUrl='" + ServerConfigurationService.getString("serverUrl") + "';");
				writer.write("config.prototype.enableVT=true;");
				writer.write("config.prototype.siteId='" + siteId +"';");
				writer.write("config.prototype.resourceId='"+ resourceid+ "';");
			}
		...
		/*
		 * 
		 * if(editor != null && !editor.equalsIgnoreCase("FCKeditor")) { // Render JavaScripts. writeExternalScripts(locale, writer);
		 * 
		 * // Render base textarea. writeTextArea(clientId, value, textareaRows, textareaColumns, writer);
		 * 
		 * // Make textarea rich text (unless textareaOnly is true). if (!"true".equals(textareaOnly)) { String toolbarScript;
		 * 
		 * if (buttonList != null) { toolbarScript = makeToolbarScript(buttonList); } else { toolbarScript = getStandardToolbarScript(buttonSet); }
		 * 
		 * // hook up configuration object writeConfigurationScript(context, component, clientId, toolbarScript, widthPx, heightPx, showXPath, locale, writer); } } else { //not as slick as the way htmlarea is rendered, but the difference in functionality
		 * doesn't all //make sense for FCK at this time since it's already got the ability to insert files and such. String collectionBase = (String) RendererUtil.getAttribute(context, component, "collectionBase"); String collectionId = null;
		 * 
		 * String connector = "/sakai-fck-connector/filemanager/connector";
		 * 
		 * if (collectionBase != null) { collectionId = collectionBase; connector += collectionBase; } else { collectionId = ContentHostingService.getSiteCollection(ToolManager.getCurrentPlacement().getContext()); }
		 * 
		 * writer.write("<table border=\"0\"><tr><td>"); writer.write("<textarea name=\"" + clientId + "_inputRichText\" id=\"" + clientId + "_inputRichText\""); if (textareaColumns > 0) writer.write(" cols=\""+textareaColumns+"\""); if (textareaRows > 0)
		 * writer.write(" rows=\""+textareaRows+"\""); writer.write(">"); if (value != null) writer.write((String) value); writer.write("</textarea>");
		 * 
		 ...
		 * 
		 * Object attchedFiles = RendererUtil.getAttribute(context, component, "attachedFiles"); if (attchedFiles != null && getSize(attchedFiles) > 0) { writeFilesArray(writer, attachmentVar, attchedFiles, LIST_ITEM_FORMAT_FCK, false);
		 * writer.write("\n"); hasAttachments = true; }
		 * 
		 ...
		 * 
		 * if (widthPx < 0) widthPx = 600; if (heightPx < 0) heightPx = 400; //FCK's toolset is larger then htmlarea and this prevents tools from ending up with all toolbar //and no actual editing area. if (heightPx < 200) heightPx = 200;
		 * 
		 ...
		 * 
		 * writer.write("\n\t\tvar collectionId = \"" + collectionId + "\";"); writer.write("\n\toFCKeditor.Config['ImageBrowserURL'] = oFCKeditor.BasePath + " + "\"editor/filemanager/browser/default/browser.html?Connector=" + connector +
		 * "&Type=Image&CurrentFolder=\" + collectionId;"); writer.write("\n\toFCKeditor.Config['LinkBrowserURL'] = oFCKeditor.BasePath + " + "\"editor/filemanager/browser/default/browser.html?Connector=" + connector +
		 * "&Type=Link&CurrentFolder=\" + collectionId;"); writer.write("\n\toFCKeditor.Config['FlashBrowserURL'] = oFCKeditor.BasePath + " + "\"editor/filemanager/browser/default/browser.html?Connector=" + connector +
		 * "&Type=Flash&CurrentFolder=\" + collectionId;"); writer.write("\n\toFCKeditor.Config['ImageUploadURL'] = oFCKeditor.BasePath + " + "\"" + connector + "?Type=Image&Command=QuickUpload&Type=Image&CurrentFolder=\" + collectionId;");
		 * writer.write("\n\toFCKeditor.Config['FlashUploadURL'] = oFCKeditor.BasePath + " + "\"" + connector + "?Type=Flash&Command=QuickUpload&Type=Flash&CurrentFolder=\" + collectionId;");
		 * writer.write("\n\toFCKeditor.Config['LinkUploadURL'] = oFCKeditor.BasePath + " + "\"" + connector + "?Type=File&Command=QuickUpload&Type=Link&CurrentFolder=\" + collectionId;");
		 * 
	...
	{
		writer.write("<script type=\"text/javascript\">var _editor_url = \"" + "/" + RESOURCE_PATH + "/" + HTMLAREA_SCRIPT_PATH + "/"
				+ "\";</script>\n");
		...
		{
			writer.write("<script type=\"text/javascript\" src=\"" + "/" + RESOURCE_PATH + "/" + HTMLAREA_SCRIPT_PATH + "/" + "lang/" + language
					+ ".js\"></script>\n");
		}
	...
	public void decode(FacesContext context, UIComponent component)
	{
		...
		if (null == context || null == component || !(component instanceof org.sakaiproject.jsf.component.InputRichTextComponent))
		{
	 ...
	 *        the client id
	 * @param toolbar
	 *        the toolbar configuration string (i.e from makeToolbarScript())
	 * @param widthPx
	 *        columns
	 * @param heightPx
	 *        rows
	 */
	protected void writeConfigurationScript(FacesContext context, UIComponent component, String clientId, String toolbar, int widthPx, int heightPx,
			String showXPath, Locale locale, ResponseWriter writer) throws IOException
	{
		...
		writeAdditionalConfig(context, component, configVar, clientId, toolbar, widthPx, heightPx, locale, writer);
	 ...
	 * @param writer
	 */
	protected void writeAdditionalConfig(FacesContext context, UIComponent component, String configVar, String clientId, String toolbar, int widthPx,
			int heightPx, Locale locale, ResponseWriter writer) throws IOException
	{
	...
	protected void writeFilesArray(ResponseWriter writer, String arrayVar, Object attchedFiles, MessageFormat format, boolean includeLabel)
			throws IOException
	{
	...
	protected String outputFiles(Map map, MessageFormat format, boolean first)
	{
		...
		for (Iterator i = map.entrySet().iterator(); i.hasNext();)
		{
			...
			{ entry.getValue(), entry.getKey() }, sb, null);
		}
