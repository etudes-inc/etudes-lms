/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/tool/tool-impl/impl/src/java/org/sakaiproject/tool/impl/SessionComponent.java $
 * $Id: SessionComponent.java 8067 2014-06-01 19:50:38Z ggolden $
 ***********************************************************************************
 *
...
public abstract class SessionComponent implements SessionManager
{
	...
	public Session startSession()
	{
		...
		if ((old != null) && (old != s))
		{
			M_log.warn("startSession(): session id conflict: " + s.getId());
		}
	 ...
	 * @inheritDoc
	 */
	public Session startSession(Session s)
	{
		// make sure it has an id
		((MySession) s).establishId();
		...
		if ((old != null) && (old != s))
		{
			M_log.warn("startSession(Session): session id conflict: " + s.getId());
		}
	...
	public Session startSession(String id)
	{
		...
		if ((old != null)  && (old != s))
		{
			M_log.warn("startSession(id): session id conflict: " + s.getId());
		}
	...
	public Session getCurrentSession()
	{
		...
		if (rv == null)
		{
			// set the id to null, indicating it's temporary nature
			rv = new MySession(null);
	...
	public class MySession implements Session, HttpSession
	{
		...
		/**
		 * Pick up an ID, if not already set.
		 */
		...
		public void invalidate()
		{
			...
			synchronized (this)
			{
				...
				// let it not be found - if the id is null, we are not remembering it
		...
		public boolean equals(Object obj)
		{
			...
			// id may be null
			return (!different(((Session) obj).getId(), getId()));
		}
		...
		{
			// id may be null
			if (this.m_id == null) return "".hashCode();
