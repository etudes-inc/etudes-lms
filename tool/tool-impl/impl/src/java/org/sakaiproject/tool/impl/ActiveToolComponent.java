/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/tool/tool-impl/impl/src/java/org/sakaiproject/tool/impl/ActiveToolComponent.java $
 * $Id: ActiveToolComponent.java 5090 2013-06-05 20:51:06Z ggolden $
 ***********************************************************************************
 *
...
public abstract class ActiveToolComponent extends ToolComponent implements ActiveToolManager
{
	...
	public void register(Tool tool, ServletContext context)
	{
		...
		if ("Sakai".equals(tool.getDispatcherStyle()))
		{
	...
	public void register(Document toolXml, ServletContext context)
	{
		...
		for (int i = 0; i < rootNodesLength; i++)
		{
			...
			if (rootElement.getTagName().equals("tool"))
			{
				...
				tool.setDispatcherStyle(rootElement.getAttribute("dispatcher").trim());
				if ("E3".equals(tool.getDispatcherStyle()))
				{
					// TODO: with Servlet 2.5, we can use getContextPath()
					tool.setE3Path("/" + context.getServletContextName());
					tool.setE3Code(rootElement.getAttribute("e3Code").trim());
				}
	...
	public class MyActiveTool extends org.sakaiproject.util.Tool implements ActiveTool
	{
		...
		public MyActiveTool(org.sakaiproject.tool.api.Tool t)
		{
			...
			this.m_dispatcherStyle = t.getDispatcherStyle();
			this.m_e3Code = t.getE3Code();
			this.m_e3Path = t.getE3Path();
		}
