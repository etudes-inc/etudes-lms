/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/tool/tool-util/servlet/src/java/org/sakaiproject/util/RequestFilter.java $
 * $Id: RequestFilter.java 12364 2015-12-23 21:18:09Z ggolden $
 ***********************************************************************************
 *
 * Copyright (c) 2005, 2006, 2007, 2015 The Sakai Foundation.
 * 
...
public class RequestFilter implements Filter
{
	...
	protected final static String DEEPEST_REQUEST = "org.sakaiproject.util.RequestFilter.deepest.request";
		...
		protected String rewriteURL(String url)
		{
			if (m_toolPlacement)
			{
				...
				if (placementId != null)
				{
					...
					if (((m_req.getServerPort() != 80) && (!m_req.isSecure())) || ((m_req.getServerPort() != 443) && (m_req.isSecure())))
					{
						full.append(":");
	...
	static class WrappedRequestFileUpload extends HttpServletRequestWrapper
	{
		...
		public void cleanup()
		{
			...
			for (Enumeration<?> e = this.getAttributeNames(); e.hasMoreElements();)
			{
				...
				Object attribute = this.getAttribute(attrName);
				if (attribute instanceof DiskFileItem)
				{
					DiskFileItem dfi = (DiskFileItem) attribute;
					if (!dfi.isInMemory())
					{
						dfi.delete();
					}
	...
	{
		// the request filter may be called a few times, nested, for a single request. Once we have been through once, we add ATTR_FILTERED to the request attributes to know that we have.
		// First time through, we do processing... and we want to assure that ThreadLocal is clear
		if (((HttpServletRequest) requestObj).getAttribute(ATTR_FILTERED) == null)
		{
		...
		WrappedRequestFileUpload reqToCleanup = null;
		try
		{
			...
			// handle file uploads
			reqToCleanup = handleFileUpload(req, resp);
			if (reqToCleanup != null) req = reqToCleanup;

			// record the deepest request object for use in the error reporter
			ThreadLocalManager.set(DEEPEST_REQUEST, req);
			...
			else
			{
				...
				try
				{
					Thread.currentThread().setName(threadName + req.getPathInfo());
					...
					// SAK-271 - Force IE8 compatibility to avoid IE9 problems
					// See: http://msdn.microsoft.com/en-us/library/ff955275(v=vs.85).aspx
					// resp.setHeader("X-UA-Compatible", "IE=10");
		...
		{
			if (reqToCleanup != null)
			{
				reqToCleanup.cleanup();
			}
	...
	public void init(FilterConfig filterConfig) throws ServletException
	{
		...
		{
			m_characterEncodingEnabled = Boolean.valueOf(filterConfig.getInitParameter(CONFIG_CHARACTER_ENCODING_ENABLED)).booleanValue();
		}
	...
	{
		if (m_characterEncodingEnabled && req.getCharacterEncoding() == null && m_characterEncoding != null && m_characterEncoding.length() > 0
				&& req.getAttribute(ATTR_CHARACTER_ENCODING_DONE) == null)
		{
	 ...
	 */
	protected WrappedRequestFileUpload handleFileUpload(HttpServletRequest req, HttpServletResponse resp) throws ServletException,
			UnsupportedEncodingException
	{
		if ((!m_uploadEnabled) || /* !FileUpload.isMultipartContent(req) */(!ServletFileUpload.isMultipartContent(req))
		...
		if (this.m_uploadMaxSize > 0) upload.setSizeMax(this.m_uploadMaxSize);
		...
		{
			M_log.warn("Upload size exceeds ceiling: " + ((upload.getSizeMax() / 1024L) / 1024L) + " > " + ((m_uploadCeiling / 1024L) / 1024L)
					+ " megs");

			upload.setSizeMax(m_uploadCeiling);
		}

//		System.out.println("RequestFilter: tracker: " + factory.getFileCleaningTracker() + " threshold: " + factory.getSizeThreshold() + " repo: "
...
//					System.out.println("RequestFilter: isFormField: " + item.getFieldName() + " : inMemory: " + item.isInMemory() + " size: " + item.getSize());
				...
				{
//					System.out.println("RequestFilter: file: " + ((DiskFileItem) item).getStoreLocation().getPath() + " : " + item.getFieldName() + " : inMemory: " + item.isInMemory() + " size: " + item.getSize());
		...
		{
			M_log.warn("Unexpected exception in upload parsing" + ex);
	...
	protected Session assureSession(HttpServletRequest req, HttpServletResponse res)
	{
		...
		ThreadLocalManager.set(SAKAI_SERVERID, suffix);
		...
		if (s == null)
		{
			// rather than a real stored session, just go with a temp. one
			// s = SessionManager.startSession();
			s = SessionManager.getCurrentSession();
			...
			// if we had a session id requested that we didn't find, record it in the session
			if (sessionId != null)
			{
				s.setAttribute("RequestFilter:requested.session.id", sessionId);
			}
		}

		else
		{
			// using a found session for the xlogin post is a really bad idea. It could happen because a user manually enters the xlogin url and logs in again without logging out, but more likely, its a problem.
			if ("POST".equals(req.getMethod()) && "/xlogin".equalsIgnoreCase(req.getPathInfo()))
			{
				M_log.warn("assureSession: POST /xlogin found a session: id: " + s.getId() + " session EID: " + s.getUserEid() + " session UID: "
						+ s.getUserId() + " request IP: " + req.getRemoteAddr());
			}
		}
