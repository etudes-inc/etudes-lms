/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/reference/library/src/webapp/js/headscripts.js $
 * $Id: headscripts.js 8042 2014-05-29 21:49:07Z rashmim $
 ***********************************************************************************
 *
...
function loadXMLDoc(url)
{
	...
	if (window.XMLHttpRequest)
	{
		...
		// and the auto parameter to indicate this is not user activity
	       if (url.indexOf("/melete/") != -1)
		     updateReq.open("GET", url + "?now=" + new Date().getTime() + "&muser=" + Math.random() + "&auto=true", true);
	       else updateReq.open("GET", url + "/" + new Date().getTime() + "-" + Math.random() + "?auto=true", true);
		updateReq.send(null);
	}
