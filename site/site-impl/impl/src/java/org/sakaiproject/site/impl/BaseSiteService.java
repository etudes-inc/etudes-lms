/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/site/site-impl/impl/src/java/org/sakaiproject/site/impl/BaseSiteService.java $
 * $Id: BaseSiteService.java 9527 2014-12-12 23:09:51Z ggolden $
 ***********************************************************************************
 *
...
public abstract class BaseSiteService implements SiteService, StorageUser
{
	...
	public Long getSiteTermId(String siteId)
	{
		String sql = "SELECT T.TERM_ID FROM ARCHIVES_SITE_TERM T WHERE T.SITE_ID = ?";
		Object[] fields = new Object[1];
		fields[0] = siteId;
		List results = SqlService.dbRead(sql, fields, null);
		if (results.size() > 0)
		{
			String termIdStr = (String) results.get(0);
	...
	public void init()
	{
		try
		{
			...
			functionManager().registerFunction(SITE_OBSERVER);
			...
			if (codes == null || prefix == null || codes.length != prefix.length)
			{
				M_log.warn("(site) etudes.roster.institution.codes and etudes.roster.institution.prefix not properly set in properties");
			}
			else
			{
				for (int i = 0; i < codes.length; i++)
				{
					this.prefixMap.put(prefix[i], codes[i]);
				}
			}
	 ...
	 * @inheritDoc
	 */
	public String getInstitutionCode(String siteId)
	{
		try
		{
			Site site = getDefinedSite(siteId);
			String[] parts = StringUtil.splitFirst(site.getTitle(), " ");
	...
	public void removeSite(Site site) throws PermissionException
	{
		...
		dbScheduleService.deleteDelayedInvocation("", site.getId());
	...
	{
		return m_storage.getSites(type, ofType, criteria, null, null, propertyCriteria, sort, page);
	}

	/**
	 * @inheritDoc
	 */
	public List getSites(SelectionType type, Object ofType, String criteria, Integer term,  String instPrefix, Map propertyCriteria, SortType sort, PagingPosition page)
	{
		return m_storage.getSites(type, ofType, criteria, term, instPrefix, propertyCriteria, sort, page);
	}
	...
	{
		return m_storage.countSites(type, ofType, criteria, null, null, propertyCriteria);
	}
	...
	{
		return m_storage.countSites(type, ofType, criteria, term, instPrefix, propertyCriteria);
	}
	 ...
	 * @inheritDoc
	 */
	public Site getSiteByTitle(String title) throws IdUnusedException
	{
		Site rv = m_storage.getByTitle(title);
		if (rv == null) throw new IdUnusedException(title);
		return rv;
	}
	 ...
	 * @inheritDoc
	 */
	public Site getTemplate(String title)
	{
		// for the site template, use "<site prefix> TEMPLATE DEV"
		String templateName = "";
		String[] parts = StringUtil.splitFirst(title, " ");
		if (parts != null && parts.length > 0)
		{
			templateName += parts[0].toUpperCase();
		}
		templateName += " TEMPLATE DEV";

		// find a site with this title
		try
		{
			Site template = getSiteByTitle(templateName);
			return template;
		}
		catch (IdUnusedException e)
		{
	...
	protected interface Storage
	{
		...
		/**
		 * Get the site with this title, or null if not found.
		 * 
		 * @param title
		 *        The site title.
		 * @return The site with this title, or null if not found.
		 */
		 ...
		 * @param term
		 *        Site term (numeric code) - if specified, only sites in this term are selected.
		 * @param instPrefix
		 *        If specified, only these sites from this institution (with this title prefix) are considered.
		 ...
		 */
		public List getSites(SelectionType type, Object ofType, String criteria, Integer term, String instPrefix, Map propertyCriteria,
				SortType sort, PagingPosition page);

		/**
		 * Count the Site objects that meet specified criteria.
		 * 
		 * @param type
		 *        The SelectionType specifying what sort of selection is intended.
		 * @param ofType
		 ...
		 * @param propertyCriteria
		 *        Additional selection criteria: sites returned will have a property named to match each key in the map, whose values match (somewhere in their value) the value in the map (may be null or empty).
		 * @return The count of Site objects that meet specified criteria.
		 */
		public int countSites(SelectionType type, Object ofType, String criteria, Integer term, String instPrefix, Map propertyCriteria);
	 ...
	 */
	protected int indexOf(String value, List<Site> siteList)
	{
	 ...
	 *        The List of sites.
	 */
	protected void sortSites(List<Site> sites)
	{
		// put the rest of the sites in term / title order
		Collections.sort(sites, new Comparator<Site>()
		{
			public int compare(Site arg0, Site arg1)
			{
				// by term (descending)
				int rv = Integer.valueOf(arg1.getTermId()).compareTo(Integer.valueOf(arg0.getTermId()));
				if (rv != 0) return rv;
	...
	/**
	 * Get all the user visible sites, in preferences order
	 * 
	 * @param userId
	 *        The user id.
	 * @param visibleSites
	 *        a list in which to return the visible sites, in order.
	 * @param hiddenSites
	 *        a list in which to return the hidden sites, in order.
	 * @return a List of the user's sites.
	 */
	public void getOrderedSites(String userId, List<Site> visibleSites, List<Site> hiddenSites)
	{
		// collect the user's sites
		@SuppressWarnings("unchecked")
		List<Site> mySites = getSites(org.sakaiproject.site.api.SiteService.SelectionType.MYSITES, null, null, null,
				org.sakaiproject.site.api.SiteService.SortType.TITLE_ASC, null);
		List<Site> excluded = new ArrayList<Site>();

		// sort by term and title - "visible" ordered ones will be pulled up, and "invisible" ones will be put into excluded later
		sortSites(mySites);
		...
		// re-order mySites to have un-ordered first, followed by the ordered ones
		if (prefOrder != null)
		{
			...
			mySites.addAll(ordered);
		}

		// collect the excluded sites
		if (prefExclude != null)
		{
			for (String id : prefExclude)
			{
				int pos = indexOf(id, mySites);
				if (pos != -1)
				{
					excluded.add(mySites.get(pos));
				}
			}
		...
		visibleSites.addAll(mySites);
		hiddenSites.addAll(excluded);
	}
}
