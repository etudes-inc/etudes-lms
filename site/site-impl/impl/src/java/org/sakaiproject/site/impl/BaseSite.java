/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/site/site-impl/impl/src/java/org/sakaiproject/site/impl/BaseSite.java $
 * $Id: BaseSite.java 7828 2014-04-18 22:29:46Z ggolden $
 ***********************************************************************************
 *
...
public class BaseSite implements Site
{
	...
	protected String m_termSuffix = null;
	...
	public BaseSite(String id, String title, String type, String shortDesc, String description, String iconUrl, String infoUrl,
			String skin, boolean published, boolean joinable, boolean pubView, String joinRole, boolean isSpecial, boolean isUser,
			String createdBy, Time createdOn, String modifiedBy, Time modifiedOn, String termSuffix, String termId)
	{
		// setup for properties
		m_properties = new BaseResourcePropertiesEdit();
	...
	public void loadAll()
	{
		...
		// next, tools from all pages, all at once - if some page is still tool-lazy
		boolean somePageToolsLazy = false;
		for (Iterator iPages = this.m_pages.iterator(); iPages.hasNext();)
		{
			BaseSitePage page = (BaseSitePage) iPages.next();
			if (page.m_toolsLazy)
			{
				somePageToolsLazy = true;
				break;
			}
		}
		if (somePageToolsLazy)
		{
		...
		// now all properties - if any are still lazy
		boolean somePropsLazy = false;
		if (((BaseResourceProperties) (this.m_properties)).isLazy())
		{
			somePropsLazy = true;
		}
		if (!somePropsLazy)
		{
			for (Iterator iPages = this.m_pages.iterator(); iPages.hasNext();)
			{
				...
				for (Iterator t = page.getTools().iterator(); t.hasNext();)
				{
					...
					if (tool.m_configLazy)
					{
						somePropsLazy = true;
						break;
					}
				}
				if (somePropsLazy) break;
			}
		}
		if (!somePropsLazy)
		{
			for (Iterator i = this.m_groups.iterator(); i.hasNext();)
			{
				BaseGroup group = (BaseGroup) i.next();
				if (((BaseResourcePropertiesEdit) group.m_properties).isLazy())
				{
					somePropsLazy = true;
					break;
				}
	...
	{
		ToolConfiguration config = getToolForCommonId(commonToolId);
		return (config != null);
	}
	...
	public Set getActiveMembers()
	{
		return getAzg().getActiveMembers();
	}
	...
	{
		// we don't always have the m_termSuffix set, so use the title if missing
		String suffix = m_termSuffix;
		if ((suffix == null) && (m_title != null))
		{
			String[] parts = StringUtil.split(m_title," ");
			suffix = parts[parts.length-1];
		}

		if (suffix == null)	return "Project";
		if ("DEV".equals(suffix)) return "Development";
		if (suffix.startsWith("W") && (suffix.length() == 3)) return "Winter 20" + suffix.substring(1, 3);
		if (suffix.startsWith("SP") && (suffix.length() == 4)) return "Spring 20" + suffix.substring(2, 4);
		if (suffix.startsWith("SU") && (suffix.length() == 4)) return "Summer 20" + suffix.substring(2, 4);
		if (suffix.startsWith("F") && (suffix.length() == 3)) return "Fall 20" + suffix.substring(1, 3);
		return "Project";
	}
	 ...
	 * @inheritDoc
	 */
	public boolean userEditedSkin()
	{
		if (getSkin() == null) return false;

		// find the template
		Site template = SiteService.getTemplate(getTitle());
		if (template != null)
		{
			if (getSkin().equals(template.getSkin())) return false;
		}
	 ...
	 * @inheritDoc
	 */
	public boolean userEditedInfo()
	{
		// this is the default on creation
		if ("/access/content/public/images/home/seagull.gif".equals(getInfoUrl())) return false;
		return true;
	}
	 ...
	 * @inheritDoc
	 */
	public boolean userEditedPubDates()
	{
		String pubDate = getProperties().getProperty("pub-date");
		if (pubDate != null) return true;
		String unpubDate = getProperties().getProperty("unpub-date");
		if (unpubDate != null) return true;
