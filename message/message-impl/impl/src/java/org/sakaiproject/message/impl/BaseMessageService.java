/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/message/message-impl/impl/src/java/org/sakaiproject/message/impl/BaseMessageService.java $
 * $Id: BaseMessageService.java 8212 2014-06-09 08:05:14Z mallikamt $
 ***********************************************************************************
 *
...
public abstract class BaseMessageService implements MessageService, StorageUser, CacheRefresher
{
	 ...
	 *
	 * @param chan
	 *        The channel to own this header.
	 * @param id
	 *        The message Id.
	 * @return The new channel header.
	 */
	protected abstract MessageChannelHeaderEdit newChannelHeader(MessageChannel chan/*, String id*/);
	 ...
	 * @return The new message header.
	 */
	protected abstract MessageChannelHeaderEdit newChannelHeader(MessageChannel chan, Element el);
	 ...
	 * @return The new channel header.
	 */
	protected abstract MessageChannelHeaderEdit newChannelHeader(MessageChannel msg, MessageChannelHeader other);
	...
	/* 
     * Extract the channel name from the full channel reference
	 * @param channelRef
	 *        The channel reference.
	 * @return The extracted channel id.
	 */
	public String extractChannelId(String channelRef)
	{
		return channelRef.substring(channelRef.lastIndexOf(Entity.SEPARATOR) + 1);
	}
	...
	public MessageChannelEdit addChannel(String ref) throws IdUsedException, IdInvalidException, PermissionException
	{
		...
		if (!m_entityManager.checkReference(ref, INVALID_CHARS_IN_CHANNEL_ID)) throw new IdInvalidException(ref);
	...
	/**
	 * Update the channel title of an existing channel
	 *
	 * @param ref
	 *        The channel reference.
	 * @param newRef
	 ...
	 *            if the user does not have permission to add a channel.
	 */
	public MessageChannelEdit updateChannel(String ref, String newRef) throws IdUsedException, IdInvalidException, PermissionException
	{
		// check the name's validity
		if (!m_entityManager.checkReference(newRef, INVALID_CHARS_IN_CHANNEL_ID)) throw new IdInvalidException(newRef);

		// check for existance
		if (m_storage.checkChannel(newRef))
		{
			throw new IdUsedException(newRef);
		}

		// check security
		unlock(SECURE_UPDATE, ref);

		MessageChannelEdit channel = m_storage.updateChannel( ref, newRef, extractChannelId(newRef));
		
		((BaseMessageChannelEdit) channel).setEvent(SECURE_UPDATE);
	...
	{
		// check security (throws if not permitted)
		return unlockCheck(SECURE_UPDATE, ref);
	...
	public void removeChannel(MessageChannelEdit channel) throws PermissionException
	{
		...
		// check security
		unlock(SECURE_UPDATE, channel.getReference());
	...
	/**
	 * Access a list of channels that are defined related to the context.
	 * 
	 * @param context
	 *        The context in which to search
	 * @return A List channels for channels withing the context.
	 */
	public List getChannels(String context)
	{
		List channels = new Vector();
		
		if ((!m_caching) || (m_channelCache == null) || (m_channelCache.disabled()))
		{
			return m_storage.getChannels(context);
		}
		...
		else
		{
			...
			synchronized (m_channelCache)
			{
				...
				channels = m_storage.getChannels(context);
				// update the cache, and mark it complete
				for (int i = 0; i < channels.size(); i++)
				{
					MessageChannel channel = (MessageChannel) channels.get(i);
	...
	public class BaseMessageChannelEdit extends Observable implements MessageChannelEdit, SessionBindingListener
	{
		...
		/** The channel header. */
		protected MessageChannelHeaderEdit m_header = null;
		...
		public BaseMessageChannelEdit(String ref)
		{
			...
			m_header = newChannelHeader(this/*, m_id*/);
		...
		public BaseMessageChannelEdit(MessageChannel other)
		{
			...
			m_id = other.getId();
			// copy the header
			if (other.getHeader() != null) m_header = newChannelHeader(this, other.getHeader());
			else m_header = newChannelHeader(this/*, "test"*/);
			// setup for properties
			m_properties = new BaseResourcePropertiesEdit();
		...
		public BaseMessageChannelEdit(Element el)
		{
			...
			for (int i = 0; i < length; i++)
			{
				...
				{
					// re-create a header
					m_header = newChannelHeader(this, element);
				}
		 ...
		 */
		public MessageChannelHeader getHeader()
		{
			return m_header;
		 ...
		 * @inheritDoc
		 */
		public Collection getGroupsAllowGetRooms()
		{
			return getGroupsAllowFunction("readgroup");
		}
		...
		public void commitMessage(MessageEdit edit, int priority)
		{
			...
			// clear out any thread local caching of this message, since it has just changed (either as a single message, or as the channel messages)
			m_threadLocalManager.set(edit.getReference(), null);
			m_threadLocalManager.set(getReference() + ".msgs", null);
		 ...
		 * @inheritDoc
		 */
		public Collection getGroupsAllowAddMessage(String permission)
		{
			return getGroupsAllowFunction(permission);
		}
		...
		/**
		 * Update the channel id of existing messages
		 * @param id
		 *        The message id.
		 * @param newRef
		 ...
		 *            If the user does not have write permission to the channel.
		 */
		public MessageEdit updateMessage(String id, String newRef) throws PermissionException
		{
			unlock(SECURE_UPDATE, getReference());

			this.m_id = extractChannelId(newRef);
			// get a new message in the info store
			MessageEdit msg = m_storage.updateMessage(this, id, newRef);

			((BaseMessageEdit) msg).setEvent(SECURE_UPDATE);
		...
		public Element toXml(Document doc, Stack stack)
		{
			...
			if (m_header != null) m_header.toXml(doc, stack);
		...
		/**
		 * Access the channel header.
		 *
		 * @return The channel header.
		 */
		public MessageChannelHeaderEdit getHeaderEdit()
		{
			return m_header;

		} // getChannelHeaderEdit
	...
	public class BaseMessageChannelHeaderEdit implements MessageChannelHeaderEdit
	{
		/** The unique (within the channel) message id. */
		//protected String m_id = null;
		 ...
		 *        The channel id.
		 */
		public BaseMessageChannelHeaderEdit(MessageChannel chan/*, String id*/)
		{
			m_channel = chan;
			//m_id = id;
		} // BaseMessageChannelHeaderEdit
		 ...
		 *        The other message header to copy.
		 */
		public BaseMessageChannelHeaderEdit(MessageChannel chan, MessageChannelHeader other)
		{
			m_channel = chan;
			//m_id = other.getId();
			m_access = other.getAccess();
		 ...
		 *        The header in XML in a DOM element.
		 */
		public BaseMessageChannelHeaderEdit(MessageChannel chan, Element el)
		{
			m_channel = chan;
			//m_id = el.getAttribute("id");
		...
		public void setGroupAccess(Collection groups) throws PermissionException
		{
			...
			if (m_channel == null)
			{
				M_log.warn("setGroupAccess() called with null channel: " + m_channel.toString());
				throw new PermissionException(m_sessionManager.getCurrentSessionUserId(), "access:channel", ((m_channel == null) ? "" : ((BaseMessageChannelEdit) m_channel).getReference()));
			}
			...
			if (removedGroups.size() > 0)
			{
				...
				// the Group objects the user has remove permission
				Collection allowedGroups = m_channel.getGroupsAllowRemoveMessage(own);
			...
			{
				// the Group objects the user has add permission
				Collection allowedGroups = m_channel.getGroupsAllowAddMessage();
		...
		public void clearGroupAccess() throws PermissionException
		{
			...
			if (m_channel == null)
			{
				M_log.warn("clearGroupAccess() called with null channel: " + ((m_channel == null) ? "" : m_channel.toString()));
				throw new PermissionException(m_sessionManager.getCurrentSessionUserId(), "access:channel", ((m_channel == null) ? "" : ((BaseMessageChannelEdit) m_channel).getReference()));
			}

			// verify that the user has permission to add in the channel context
			boolean allowed = (m_channel != null) && (m_channel).allowAddChannelMessage();
			if (!allowed)
			{
				throw new PermissionException(m_sessionManager.getCurrentSessionUserId(), "access:channel", ((BaseMessageChannelEdit) m_channel).getReference());
			}
	...
	protected interface Storage
	{
		 ...
		 * Keep a new channel.
		 */
		public MessageChannelEdit updateChannel(String ref, String newRef, String context);
		...
		public MessageEdit updateMessage(MessageChannel channel, String id, String newRef);
		...
		public List getChannels(String root);
