/**********************************************************************************
 * $URL: https://source.etudes.org/svn/etudes/source/trunk/portal/portal-util/util/src/java/org/sakaiproject/util/ErrorReporter.java $
 * $Id: ErrorReporter.java 8064 2014-06-01 18:21:42Z ggolden $
 ***********************************************************************************
 *
...
public class ErrorReporter
{
	...
	{
		// if the "deepest" request is set, use it.
		// This will have been established deeper in the request-forward stack, and may have pulled into itself the request parameters.
		HttpServletRequest deepReq = (HttpServletRequest) ThreadLocalManager.get(RequestFilter.DEEPEST_REQUEST);
		if (deepReq != null) req = deepReq;
	...
	private String requestDisplay(HttpServletRequest request)
	{
		...
		try
		{
			...
			if (session != null)
			{
				...
				sb.append(":    uid:" + session.getUserId() + "\n");
				sb.append(":    eid:" + session.getUserEid() + "\n");
				sb.append(rb.getString("bugreport.session.creation")).append(
						session.getCreationTime()).append(" " + fmt.format(new Date(session.getCreationTime())) + " ").append("\n");
				sb.append(rb.getString("bugreport.session.lastaccess")).append(
						session.getLastAccessedTime()).append(" " + fmt.format(new Date(session.getLastAccessedTime())) + " ").append("\n");
